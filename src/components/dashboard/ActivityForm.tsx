...
export const ActivityForm = ({ onClose, activityId }: ActivityFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "🚫 Activity Failed to Record",
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-xl max-h-[85vh] overflow-y-auto divine-glow bg-card/95 backdrop-blur-sm border-primary/20">
        ...
        <Spin spinning={activityFormLoading}>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <Label
                  ...
                  className="text-base font-medium text-foreground"
                >
                  Lecture Hearing Duration (minutes)
                </Label>
              </div>
              <div className="relative">
                <Input
            ...
            <div className="space-y-2">
              <Label
                htmlFor="lectureDesciption"
                className="text-base font-medium text-foreground"
              >
                Lecture Description
              </Label>
              <Textarea
                id="lectureDesciption"
                value={formData.lectureDesciption}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    lectureDesciption: e.target.value,
                  }))
                }
                className="py-3 transition-sacred focus:divine-glow border-accent/20"
            ...
            <div className="space-y-2">
              <Label
                htmlFor="readingDesciption"
                className="text-base font-medium text-foreground"
              >
                Reading Description
              </Label>
              <Textarea
                id="readingDesciption"
                value={formData.readingDesciption}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    readingDesciption: e.target.value,
                  }))
                }
                className="py-3 transition-sacred focus:divine-glow border-accent/20"
                placeholder="Describe your reading"
              />
            </div>
              ...
              <div className="space-y-3 max-h-48 overflow-y-auto">
                ...
                {formData.preachingContacts.length === 0 && (
                  <p className="text-center text-muted-foreground py-4 text-sm">
                    No preaching contacts added yet. Click "Add Contact" to
                    start tracking.
                  </p>
                )}
              </div>
